...
- **Design system guide** (tokens, components, chart conventions): auto-applied via
  [.github/instructions/smartbid-design-system.instructions.md](.github/instructions/smartbid-design-system.instructions.md)
  whenever editing app `.tsx` / `.scss`. It is the source of truth for look & feel.
- **UI/UX Pro Max skill** (design inspiration DB): `.github/prompts/ui-ux-pro-max.prompt.md`.
  Use for layout/UX/chart-type ideas only — SmartBid tokens & stack always win. Its Python
  search scripts are optional (Python not required; read the `data/*.csv` files directly).
